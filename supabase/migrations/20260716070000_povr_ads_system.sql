...
-- POVR Ads System
-- ============================================================================
-- Real advertising platform: advertiser accounts funded by on-chain DHB
-- top-ups, campaigns with badge-tier/behavioral targeting, creatives with an
-- admin review queue, HMAC-token-gated impression/click tracking, per-viewer
-- frequency capping, daily stat rollups and a viewer revenue-share ledger.
--
-- Identity follows the house wallet-header convention:
-- public.get_request_wallet_address() (x-wallet-address header, lowercased).
-- Money columns (balances, spend) are writable ONLY by service-role edge
-- functions: clients get column-limited INSERT/UPDATE grants and guarded
-- status transitions, so no client can mint credit or un-spend a budget.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Helper: normalize wallet_address to lowercase on write (defense in depth —
-- RPCs and RLS all compare lowercased).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1. Advertiser accounts (prepaid USD credit, funded by DHB top-ups)
-- ----------------------------------------------------------------------------
...
-- Clients may only touch profile fields; money columns are service-role only.
...
CREATE TRIGGER ad_accounts_lower_wallet
...
-- ----------------------------------------------------------------------------
-- 2. Top-up ledger (append-only; written by ads-topup after on-chain verify)
-- ----------------------------------------------------------------------------
...
CREATE TABLE public.ad_campaigns (
  ...
  end_at           timestamptz,
  -- Targeting document — every dimension backed by real queryable data:
  -- { tiers: text[], followerMin: int, followerMax: int, languages: text[],
  --   premium: bool, communities: uuid[], behaviors: text[]
  --   ('tippers'|'ppv_buyers'|'stakers'|'streamers'),
  --   categories: text[] (contextual), followedCreators: text[] (wallets) }
...
-- Guard client-side status transitions. Service-role calls carry no wallet
-- header, so they bypass the whitelist (admin approve/reject, spend engine).
...
AS $$
DECLARE
  caller text := public.get_request_wallet_address();
BEGIN
  IF caller IS NULL OR caller = '' THEN
    RETURN NEW; -- service role / direct SQL
  END IF;

  -- Never let a wallet-header caller move money or review fields.
...
CREATE TRIGGER ad_campaigns_lower_wallet
...
-- ----------------------------------------------------------------------------
-- 4. Creatives (uploaded ad content; re-enters review when content changes)
-- ----------------------------------------------------------------------------
...
-- Any client edit to creative content sends it back to review.
...
-- ----------------------------------------------------------------------------
-- 5. Events (append-only billing log; service-role writes via RPC only).
--    No FKs so billing history survives creative/campaign deletion.
-- ----------------------------------------------------------------------------
...
-- Advertisers can audit raw events for their own campaigns.
...
-- One impression + one click max per served ad.
...
-- ----------------------------------------------------------------------------
-- 6. Daily stat rollups (fast dashboards; service-role upserts)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 7. Frequency caps (per viewer x campaign x day; service-role only)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 8. Viewer earnings ledger (POVR revenue share; payouts wired later)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 9. Materialized follows audiences ("followers of creator X")
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10. System config (service-role only; rates intentionally not public)
-- ----------------------------------------------------------------------------
...
INSERT INTO public.ad_config (key, value) VALUES
  ('viewer_share_bps', '5000'::jsonb),          -- viewer's cut of each impression (basis points)
...
-- ----------------------------------------------------------------------------
-- 11. Storage bucket for ad creative media
-- ----------------------------------------------------------------------------
...
-- Browser storage calls can't carry the x-wallet-address header, so this
-- follows the proven store-media pattern (bucket-scoped, public bucket).
-- Abuse is bounded by the mime/size limits above + admin review before any
-- creative can serve.
...
-- ============================================================================
-- RPCs — the spend engine. SECURITY DEFINER, revoked from clients; only the
-- service-role edge functions may execute (except ads_estimate_audience,
-- which returns aggregate counts only and powers the campaign wizard).
-- ============================================================================

-- Credit an account after a verified on-chain DHB transfer. Idempotent per tx.
...
-- Record a billable impression atomically: event + campaign spend + account
-- debit + frequency + daily rollup + viewer earnings. Returns dedupe/budget info.
...
BEGIN
  ...
  UPDATE ad_campaigns
  SET spent_usd = spent_usd + p_price
  WHERE id = p_campaign
...
-- Record a click (free under CPM; still deduped + rolled up).
...
-- Audience size estimate for the campaign wizard. Aggregate counts only.
-- Tier thresholds mirror src/lib/staking-badges.ts (canonical badge system).
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE (v_tiers IS NULL OR l.tier = ANY(v_tiers))
      ...
      AND (v_langs IS NULL OR EXISTS (
        ...
        WHERE lower(p.wallet_address) = l.account
          AND p.preferences ->> 'language' = ANY(v_langs)))
      AND (v_premium IS NOT TRUE OR EXISTS (
        SELECT 1 FROM premium_subscriptions ps
        WHERE lower(ps.wallet_address) = l.account
          AND ps.status IN ('active','trialing')))
      AND (v_communities IS NULL OR EXISTS (
        SELECT 1 FROM community_members cm
        WHERE lower(cm.wallet_address) = l.account
          AND cm.community_id = ANY(v_communities)
  ...
  FROM (
    SELECT tier, count(*) AS cnt FROM filtered GROUP BY tier
  ) g;
...
-- Lock down the spend engine; estimator stays client-callable.
