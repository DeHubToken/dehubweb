-- Community chat @mentions and @here
-- ==================================
-- An @mention in a community chat notified nobody: the notifications panel is
-- fed by the DeHub API plus public.custom_notifications, and community chat
-- wrote to neither. This adds the fan-out for both a direct @handle and an
-- @here broadcast.
--
-- Why an RPC rather than a trigger on community_chat_messages: resolving a
-- @handle to a wallet needs the account directory, which lives in the DeHub API
-- and not in this database. The client resolves the handles it can see and
-- passes wallets in; the server then decides who may actually be notified. The
-- client is never trusted with the @here recipient list — that is expanded here
-- from the membership table.
--
-- Caller identity is get_request_wallet_address(), i.e. the x-wallet-address
-- header, consistent with every other community RPC.

-- @here is the one action that can reach every member at once, and the request
-- that asked for it named the abuse risk. The limit is checked against the
-- notifications already written, so there is no extra state to keep.
CREATE INDEX IF NOT EXISTS custom_notifications_here_rate_idx
  ON public.custom_notifications (actor_address, reference_id, created_at)
...
DECLARE
  ...
  batch       INTEGER;
BEGIN
  -- Posting rights are the floor: whoever may not speak here may not make
  -- everyone's notification bell ring either.
  ...
  -- The message anchors the notification and proves the mention came from a real
  -- post by this actor rather than from a bare RPC call.
  SELECT * INTO msg
  ...
  -- Enough of the message to recognise it, without storing a second copy.
  ...
  -- Direct @handles. Skipped entirely after an @here, which already reached
  -- every active member — a second row for the same message would double up.
