-- Unbreak the minute snapshot: rename the local variable that shadowed a column.
--
-- sample_market now selects dex_sell_positions.side in its loop query, and the
-- function already declared a local `side`. PL/pgSQL refuses the ambiguous
-- reference, so every cron tick failed to plan and the shared market payload
-- froze at its last good minute. The variable is now order_side; nothing else
