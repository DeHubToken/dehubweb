...
-- Ported from the open-source Rilable pipeline (github.com/rbrown101010/rilable),
-- re-hosted on Supabase: builder-api generates/edits apps, builder-serve hosts them.
--
-- All four tables are written ONLY by edge functions via the service role.
-- RLS is enabled with NO policies on purpose: wallet identity comes from
-- x-wallet-address/x-dehub-token headers verified server-side, so anon
-- PostgREST access (which could only trust a spoofable header) is closed off.
...
create table if not exists public.builder_projects (
  ...
  -- queued | generating | publishing | live | updating | error
