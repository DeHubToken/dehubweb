...
// Add watermark using Canvas API (more robust than imagescript)
async function addWatermark(imageBase64: string): Promise<string> {
  try {
    // Load the main image
    const mainImage = await loadImage(imageBase64);
    ...
    const logoDataUrl = `data:image/png;base64,${DEHUB_LOGO_BASE64}`;
    ...
    console.log('Watermark applied successfully via Canvas API');
