...
serve(async (req) => {
    ...
    try {
        ...
        // generated by scripts/generate-og-cards.mjs into public/og/ and served
        // as ordinary static assets.
        //
        // These used to live on the Lovable asset host at
        // /__l5e/assets-v1/<uuid>/<name>.png. That URL space does not survive
        // the July 2026 Cloudflare migration — the worker routes /_-prefixed
        // paths to the ASSETS binding, whose SPA fallback answered every one of
        // them with 200 text/html — so each of these routes was handing
        // crawlers the app shell where a PNG was promised.
        const ogCard = (name: string) => `https://dehub.io/og/${name}.png`;
        const STATIC_ROUTE_IMAGES: Record<string, string> = {
            "features":    ogCard("features"),
            "pricing":     ogCard("pricing"),
            "creator":     ogCard("creator"),
            "editor":      ogCard("editor"),
            "prompt":      ogCard("prompt"),
            "premium":     ogCard("premium"),
            "affiliate":   ogCard("affiliate"),
            "governance":  ogCard("governance"),
            "leaderboard": ogCard("leaderboard"),
            "top-100":     ogCard("top-100"),
            "music":       ogCard("music"),
            "tv":          ogCard("tv"),
            "agents":      ogCard("agents"),
            "assistant":   ogCard("assistant"),
            "creators":    ogCard("creators"),
            "bridge":      ogCard("bridge"),
            // Previously absent, so these four fell back to the shared brand
            // logo and shared as the same picture as each other.
            "work":        ogCard("work"),
            "radio":       ogCard("radio"),
            "glossary":    ogCard("glossary"),
            "jobs":        ogCard("jobs"),
            "guides/best-decentralized-social-media": ogCard("guides-best-decentralized-social-media"),
        };
