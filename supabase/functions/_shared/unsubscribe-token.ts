...
// it here just before the send. sendTemplateEmail does that now.
//
// Restored after the 2026-09-01 email rewrite dropped it along with the queue
// it used to live in. @lovable.dev/email-js@0.1.0 still carries
// `unsubscribe_token` in EmailSendRequest and does not validate it client-side,
// so the rule is the send API's and unchanged; the SDK will happily send
// without one and let the API refuse it. Sending a token the API no longer
// needs costs nothing, so this is the safe direction.
//
