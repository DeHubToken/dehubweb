...
- **Badges** are a 13-rung ladder tied to DHB held: Crab, Lobster, Piranha, Tortoise, Cobra, Octopus, Crocodile, Dolphin, Tiger Shark, Great White Shark, Killer Whale, Blue Whale, Megalodon. The rungs are pegged in dollars, so the DHB each one costs moves with the price — never quote a fixed DHB figure, look the live one up. A tier once earned is kept for as long as the holder stays at or above what it cost them.
...
- **"My post is not showing."** — Minting is optional. A post published without minting shows straight away; one the creator chose to mint reaches the feed once the mint confirms. Look the post up before agreeing anything is broken.
