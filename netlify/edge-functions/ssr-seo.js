...
async function getDocsContent(request, route) {
  if (_docsContentCache.has(route)) return _docsContentCache.get(route);
  let content = null;
  try {
    const origin = new URL(request.url).origin;
    const res = await fetch(`${origin}/docs-content/${route.replace(/\//g, '-')}.json`, { headers: { Accept: 'application/json' } });
...
async function getBlogContent(request, slug) {
  if (_blogContentCache.has(slug)) return _blogContentCache.get(slug);
  let content = null;
  try {
    const origin = new URL(request.url).origin;
    const res = await fetch(`${origin}/blog-content/${slug}.json`, { headers: { Accept: 'application/json' } });
    if (res.ok) content = await res.json();
  } catch (e) {
...
export default async (request, context) => {
  ...
  if (blogMatch) {
    ...
    if (post) {
      ...
      const content = await getBlogContent(request, slug);
  ...
  if (docsMatch) {
    ...
    if (Object.hasOwn(DOCS_PAGES, route)) {
      const content = await getDocsContent(request, route);
