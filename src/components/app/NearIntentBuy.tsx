...
export function NearIntentBuy({ tokensToReceive }: { tokensToReceive: number }) {
  ...
  const estimate = <p className="text-xs text-zinc-400">{minutes ? t('nearBuy.swapEstimate', { minutes }) : t('nearBuy.estimateUnknown')} {t('nearBuy.confirmationTime')}</p>;
  ...
  return <section data-page-bento className="bg-zinc-900 rounded-2xl p-4 space-y-4" aria-label={t('nearBuy.title')}>
    ...
    {purchase ? <div className="space-y-3">
      <div aria-live="polite" className="rounded-xl border border-white/10 p-3 space-y-2">
        ...
        <p className="text-zinc-300 text-sm">{t(`nearBuy.detail_${phase}`)}</p>
      ...
      <p className="text-sm text-white">{t(canSendPayment(purchase, flow.now) ? 'nearBuy.sendExact' : 'nearBuy.paymentSummary', { amount: purchase.amountInFormatted || '', symbol: purchase.originSymbol || '', chain: paymentChainName(purchase.originBlockchain || '') })}</p>
      ...
      <div className="rounded-lg bg-zinc-800 p-3 break-all font-mono text-xs text-white select-all">{purchase.depositAddress}</div>
      {canSendPayment(purchase, flow.now) && <Button variant="glass" className="w-full" onClick={() => copy(purchase.depositAddress)}>{t('nearBuy.copyPayment')}</Button>}
      ...
      <p className="text-xs text-zinc-400 break-all">{t('nearBuy.refundReceipt', { address: purchase.refundTo || '' })}</p>
      ...
      {selected && <>
        <label className="block text-sm text-zinc-300">{t('nearBuy.refundAddress', { chain: paymentChainName(selected.blockchain) })}<Input value={flow.refund} onChange={e => flow.setRefund(e.target.value)} disabled={busy === 'create'} placeholder={t('nearBuy.refundPlaceholder')} className={`mt-1 ${field}`} /></label>
        ...
        <Button variant="glass" className="w-full" disabled={flow.loading || flow.historyFailed || !!busy || !validDhbAmount(amount) || (!!quote && (!flow.refund.trim() || !agreed))} onClick={() => quote ? flow.create() : flow.price()}>{busy ? t('nearBuy.loading') : quote ? t('nearBuy.paymentAction') : t('nearBuy.getQuote')}</Button>
      </>}
    </>}
