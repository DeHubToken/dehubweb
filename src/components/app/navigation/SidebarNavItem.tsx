    ...
    >
      {showAvatar ? (
        <Avatar className={cn(
          ...
          {avatarUrl && (
            <AvatarImage src={avatarUrl} alt="Profile" className="object-cover rounded-full" />
          )}
          <AvatarFallback className="bg-zinc-700 text-white font-medium text-sm rounded-full">
            {avatarFallback || 'U'}
          </AvatarFallback>
        </Avatar>
      ) : (
