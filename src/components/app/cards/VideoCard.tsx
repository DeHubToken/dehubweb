  ...
  return (
    ...
    >
      ...
      <div 
        ...
        tabIndex={0}
        data-no-navigate
        className={`relative bg-black cursor-pointer group/thumb outline-none overflow-hidden transition-all duration-300 ${isImmersive ? 'rounded-none' : 'rounded-md'} ${isFullscreen ? 'fixed inset-0 z-[9999] w-screen h-screen rounded-none flex items-center justify-center' : (isImmersive && showComments ? 'aspect-[2/1]' : 'aspect-video')}`}
      ...
      >
        ...
        {!(duration > 0 && showControls) && (
          <div className={`absolute ${video.isAudio ? 'top-2 left-2 text-xs text-white font-medium z-10' : 'bottom-2 right-2 bg-black/40 backdrop-blur-[24px] saturate-[180%] px-1.5 py-0.5 rounded border border-white/10 text-xs text-white font-medium z-10'}`}>
            {isPlaying && duration > 0 ? formatTime(Math.max(0, duration - currentTime)) : video.duration}
          </div>
        )}
        </>}
