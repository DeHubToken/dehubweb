 ...
 * appearance theme is set to "lavalamp". Sits behind all app content.
 *
 * The field itself is the 21st.dev "Mesh drift" WebGL shader (zero deps, one
 * canvas). Unlike the generator's fixed 8-colour recipe, the palette here is
 * derived from the theme colour picked in Settings (hue / White / Black /
 * Rainbow), so it slots into the same colour-selector logic as Cosmic, Hazy
 * Nights and Swarms.
 */
export function LavaLampBackground() {
  const { theme, themeHues, brandColors } = useAppTheme();
  if (theme !== 'lavalamp') return null;
  ...
  return (
    <div
      ...
      style={{ zIndex: 0 }}
    >
      <LavaLampCanvas colorValue={themeHues.lavalamp ?? 20} brandColors={brandColors} />
    </div>
  );
}

// Saturation / lightness a normal hue renders its blobs at in this shader.
const LAVA_BASELINE = { saturation: 0.9, lightness: 0.55 };

type Palette = { colors: [number, number, number][]; count: number };

/**
 * Build the 8-slot metaball palette from a resolved theme colour. The shade()
 * pass weights colour[0] as the ambient background, so it is always the
 * darkest stop — that keeps the classic lava-lamp look (dark tank, glowing
 * blobs, hot near-white core) whatever hue is chosen.
 */
function buildLavaPalette(spec: ThemeColorSpec): Palette {
  const pad = (stops: [number, number, number][]): [number, number, number][] => {
    const out = stops.slice();
    while (out.length < 8) out.push(stops[stops.length - 1]);
    return out;
  };
  ...
  if (spec.kind === 'rainbow') {
    // Dark tank + a full spectrum of blobs, one hue per metaball.
    const stops: [number, number, number][] = [hslToRgb(0, 0, 0.04)];
    for (let i = 0; i < 7; i++) {
      stops.push(hslToRgb(i / 7, 0.85, 0.55));
    }
    return { colors: stops as [number, number, number][], count: 8 };
  }

  if (spec.kind === 'brand') {
    // Dark tank + the profile-picture colours as glowing blobs, plus a hot
    // near-white core tinted by the dominant colour — same lava-lamp anatomy.
    const dom = spec.brand[0] ?? [1, 1, 1];
    const hot: [number, number, number] = [dom[0] * 0.25 + 0.75, dom[1] * 0.25 + 0.75, dom[2] * 0.25 + 0.75];
    const stops: [number, number, number][] = [hslToRgb(0, 0, 0.04), ...spec.brand, hot];
    return { colors: pad(stops), count: Math.min(stops.length, 8) };
  }

  // Single hue: dark base → saturated mid glow → bright → hot near-white core.
  const h = (((spec.hue % 360) + 360) % 360) / 360;
  const s = spec.saturation;
  return {
    colors: pad([
      hslToRgb(h, s, 0.05),
      hslToRgb(h, s, 0.3),
      hslToRgb(h, s * 0.95, 0.62),
      hslToRgb(h, s * 0.55, 0.9),
    ]),
    count: 4,
  };
}
...
// Field parameters (everything except the palette, which the theme drives).
const UNIFORMS = {
  scale: 1.16,
  intensity: 0.34,
  paramA: 0.5,
  ...
  offsetY: 0.0,
  drift: 0.0,
  // Pointer interaction: the wax swirls into a soft vortex around the cursor
  // (effect 2 = rotate), matching the gentle mouse response Hazy Nights has.
  cursorEnabled: true,
  cursorEffect: 2.0,
  cursorStrength: 0.65,
  cursorRadius: 0.46,
...
function LavaLampCanvas({ colorValue, brandColors = [] }: { colorValue: number; brandColors?: string[] }) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  // Colour is pushed straight into shader uniforms so slider drags don't tear
  // down and rebuild the whole WebGL scene on every tick.
  const spec = resolveThemeColor(colorValue, LAVA_BASELINE, brandColors);
  const glRef = useRef<WebGLRenderingContext | null>(null);
  const programRef = useRef<WebGLProgram | null>(null);
  const colorsLocRef = useRef<WebGLUniformLocation | null>(null);
  const redrawRef = useRef<(() => void) | null>(null);
  const paletteRef = useRef<Palette>(buildLavaPalette(spec));
  const brandKey = spec.brand.map((c) => c.join(',')).join(';');

  // Live colour updates: rebuild the palette and re-upload without touching the
  // render loop. Repaint immediately so reduced-motion (static frame) sees it.
  useEffect(() => {
    paletteRef.current = buildLavaPalette(spec);
    const gl = glRef.current;
    const colorsLoc = colorsLocRef.current;
    if (!gl || !colorsLoc || !programRef.current) return;
    gl.useProgram(programRef.current);
    gl.uniform3fv(colorsLoc, new Float32Array(paletteRef.current.colors.flat()));
    redrawRef.current?.();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [spec.kind, spec.hue, spec.saturation, spec.lightness, brandKey]);
  ...
  useEffect(() => {
    ...
    colorsLocRef.current = uni.colors;
    gl.uniform3fv(uni.colors, new Float32Array(paletteRef.current.colors.flat()));
    gl.uniform4f(
      uni.shape,
      UNIFORMS.scale,
      UNIFORMS.intensity,
    ...
    let inView = true;
    // Paused while docs/blog composites its glass over the canvas (see
    // background-gate) — no point running the shader under heavy blur.
    ...
    function requestRender() {
      if (!disposed && visible && inView && !pausedByGate && raf === 0) {
    ...
    redrawRef.current = () => {
      ...
      gl.uniform4f(
        ...
        canvas.width,
        canvas.height,
        ((performance.now() - start) / 1000) * UNIFORMS.timeScale,
        paletteRef.current.count,
      );
      gl.drawArrays(gl.TRIANGLES, 0, 3);
    };
