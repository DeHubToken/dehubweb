  ...
  return (
    <tr className="text-zinc-400 transition-colors hover:bg-white/[0.02]">
      ...
      <td className="py-4 text-sm">
        ...
        <p className="text-zinc-600">{t('subscriptions.to')}</p>
        <p>{sub.isLifetime ? t('subscriptions.never') : safeFormat(sub.endDate, 'dd MMM yy')}</p>
      </td>
      <td className="py-4">
        {isExpired ? (
          <span className="text-red-400 text-sm">{t('subscriptions.expired')}</span>
        ) : sub.isActive ? (
          <span className="text-emerald-400 text-sm">{t('subscriptions.active')}</span>
        ) : (
          <span className="text-zinc-500 text-sm">{t('subscriptions.inactive')}</span>
        )}
      </td>
      <td className="py-4 text-sm">
        {!isExpired && sub.isActive ? (
          <span className="text-zinc-400 flex items-center gap-1">
            ...
            {sub.isLifetime ? t('subscriptions.durationLifetime') : t('subscriptions.daysLeft', { count: daysLeft })}
          </span>
        ) : (
...
export function SubscriptionsTab() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16">
        ...
        <p className="text-zinc-500 text-sm">{t('subscriptions.loadingSubscriptions')}</p>
      </div>
    );
  }
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-zinc-400 text-lg font-medium">{t('subscriptions.signInToView')}</p>
        <p className="text-zinc-500 text-sm mt-1">{t('subscriptions.connectWalletHint')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <h2 className="text-lg font-semibold text-white">{t('subscriptions.yourSubscriptions')}</h2>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Active count */}
        <div data-page-bento className={cardClass}>
          <span className="text-zinc-400 text-sm">{t('subscriptions.activeSubscriptions')}</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-bold text-white">{activeSubscriptions.length}</span>
            <span className="text-zinc-500 text-sm">{t('subscriptions.creators')}</span>
          </div>
        </div>
        ...
        {/* My plans count */}
        <div data-page-bento className={cardClass}>
          <span className="text-zinc-400 text-sm">{t('subscriptions.yourPlansAsCreator')}</span>
          <div className="flex items-baseline gap-2 mt-2">
            <span className="text-3xl font-bold text-white">{myPlans.length}</span>
            <span className="text-zinc-500 text-sm">{t('subscriptions.plans')}</span>
          </div>
        </div>
      </div>
      ...
      {subscriptions.length === 0 ? (
        <div data-page-bento className={cn(cardClass, "p-8 text-center")}>
          ...
          <p className="text-zinc-400 text-lg font-medium">{t('subscriptions.noneYet')}</p>
          <p className="text-zinc-500 text-sm mt-1">{t('subscriptions.noneYetHint')}</p>
        </div>
      ) : (
        <div data-page-bento className={cardClass}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-white font-semibold">{t('subscriptions.subscriptionList')}</span>
            <span className="text-zinc-500 text-sm">{t('subscriptions.totalCount', { count: subscriptions.length })}</span>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className={cn("text-zinc-500 text-xs", tableHeaderBorder)}>
                  ...
                  <th className="text-left font-normal pb-3">{t('subscriptions.creator')}</th>
                  <th className="text-left font-normal pb-3">{t('subscriptions.plan')}</th>
                  <th className="text-left font-normal pb-3">{t('subscriptions.price')}</th>
                  <th className="text-left font-normal pb-3">{t('subscriptions.period')}</th>
                  <th className="text-left font-normal pb-3">{t('subscriptions.status')}</th>
                  <th className="text-left font-normal pb-3">{t('subscriptions.remaining')}</th>
                </tr>
              </thead>
