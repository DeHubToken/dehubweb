 ...
 *
 * The sheet itself, split out of `AudioSpacesModal` so the app shell can mount a
 * gate instead of all this. Nothing here is on the boot path: it is reached only
 * through `React.lazy`, and only once a stage is open or the sheet is asked for.
 * Keep it that way — a static import of this file from anywhere the shell
 * reaches puts ~250 KB back in front of first paint, silently, and the entry
 * bundle guard will not catch it because no single dependency is heavy.
 *
...
export default AudioSpacesModalBody;
