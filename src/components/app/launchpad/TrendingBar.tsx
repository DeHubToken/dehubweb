              ...
              >
                ...
                <div className="h-7 w-7 rounded-lg overflow-hidden border border-white/10 bg-white/5 shrink-0">
                  {t?.image_url
                    ...
                    : <div className="h-full w-full flex items-center justify-center text-white/40 text-[10px] font-bold">—</div>}
                </div>
                <div className="flex flex-col">
                  <span className="text-white text-xs font-semibold leading-tight">{t ? `$${t.symbol}` : '—'}</span>
                  <span className="text-white/50 text-[10px] tabular-nums flex items-center gap-1 leading-tight">
                    <TrendingUp className="h-2.5 w-2.5" />{t ? fmtUsd(t.volume_24h) : '—'}
                  </span>
                </div>
              </Link>
            );
