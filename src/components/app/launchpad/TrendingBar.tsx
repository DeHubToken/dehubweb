  ...
  return (
    <div className="rounded-2xl bg-black/60 backdrop-blur-[24px] border border-white/10 overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-white/10">
        ...
        <span className="text-white text-xs font-semibold uppercase tracking-wide">Trending</span>
        <span className="text-white/40 text-[10px]">Top 20 · Volume & community activity</span>
      </div>
        ...
        <div className="flex gap-2 p-2 min-w-max">
          {tokens.map((t, i) => (
            <Link
              key={t.id}
              to={`${base}/${t.id}`}
              className="group flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 px-2.5 py-1.5 transition-colors shrink-0"
            >
              ...
              <div className="h-7 w-7 rounded-lg overflow-hidden border border-white/10 bg-white/5 shrink-0">
                {t.image_url
                  ? <img src={t.image_url} alt={t.symbol} className="h-full w-full object-cover" loading="lazy" />
                  : <div className="h-full w-full flex items-center justify-center text-white/60 text-[10px] font-bold">{t.symbol.slice(0, 2)}</div>}
              </div>
              <div className="flex flex-col">
                <span className="text-white text-xs font-semibold leading-tight">${t.symbol}</span>
                <span className="text-white/50 text-[10px] tabular-nums flex items-center gap-1 leading-tight">
                  <TrendingUp className="h-2.5 w-2.5" />{fmtUsd(t.volume_24h)}
                </span>
              </div>
            </Link>
          ))}
