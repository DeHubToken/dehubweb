 ...
 * Event Chat
 * ===========
 * Live chat for events. Mirrors CommunityChat UI.
 */
...
function ChatAvatar({ src, address, name }: { src?: string | null; address?: string; name: string }) {
  ...
  return (
    <Avatar className="w-7 h-7">
      {activeSrc && !cdnFailed && (
        <AvatarImage src={activeSrc} onError={() => failed ? setCdnFailed(true) : setFailed(true)} />
      )}
...
function ChatReactions({
  reactions, currentUserAddress, onReact, onRemoveReaction, messageId,
}: {
  ...
  return (
    <div className="flex flex-wrap gap-0.5 mt-0.5">
      {entries.map(([emoji, addresses]) => {
        const mine = currentUserAddress ? addresses.some(a => a.toLowerCase() === currentUserAddress.toLowerCase()) : false;
        return (
...
export function EventChat({ eventId }: EventChatProps) {
  ...
  const { isAuthenticated, walletAddress, openLoginModal, user } = useAuth();
  ...
  const { messages, isLoading, sendMessage, editMessage, addReaction, removeReaction } = useEventChat(eventId);
  ...
  const handleReact = useCallback((messageId: string, emoji: string) => {
    ...
    const myReaction = walletAddress && msg?.reactions?.[emoji]?.some(a => a.toLowerCase() === walletAddress.toLowerCase());
    if (myReaction) removeReaction(messageId, emoji);
  ...
  return (
    <div className="flex flex-col" style={{ height: '340px', minHeight: '200px' }}>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                          <ChatReactions reactions={msg.reactions} currentUserAddress={walletAddress || undefined} onReact={addReaction} onRemoveReaction={removeReaction} messageId={msg.id} />
                        )}
                      </div>
                      {isAuthenticated && (
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-0.5 flex-shrink-0 mt-0.5">
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button onClick={() => setReplyTo(msg)} className="p-0.5 text-zinc-500 hover:text-white transition-colors rounded">
                            ...
                            <PopoverContent side="top" align="end" className="w-auto p-1 bg-zinc-800 border-zinc-700 rounded-xl">
                              <div className="flex gap-0.5">
                                {QUICK_EMOJIS.map((emoji) => {
                                  const isActive = walletAddress && msg.reactions?.[emoji]?.some(a => a.toLowerCase() === walletAddress.toLowerCase());
                                  return (
                                    <button key={emoji} onClick={() => handleReact(msg.id, emoji)}
                                      className={`w-8 h-8 flex items-center justify-center text-sm rounded-lg transition-colors ${isActive ? 'bg-white/15 ring-1 ring-white/30' : 'hover:bg-zinc-700'}`}>
                                      {emoji}
                                    </button>
                                  );
                                })}
