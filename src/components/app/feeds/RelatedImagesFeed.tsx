...
export function RelatedImagesFeed({ currentPostId }: RelatedImagesFeedProps) {
  ...
  // Served POVR ad for the first slot (house ad NFT below is the fallback)
  const { data: servedAdList = [] } = useServedAds('related', { count: 1 });
  const servedAd = servedAdList[0] ?? null;

  // Fetch the fallback house ad video
  ...
  return (
    <div data-feed-root className="flex flex-col gap-4 pt-[30px]">
      {/* First slot: served POVR ad; falls back to the house ad NFT */}
      {servedAd ? (
      ...
      {/* Latest Images */}
      {images.map((img) => (
        <div key={img.id} data-feed-item className="rounded-xl bg-white/[0.03] p-3">
          <ImageCard post={img} />
        </div>
      ))}
