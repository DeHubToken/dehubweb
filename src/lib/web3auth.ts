...
export async function disconnectWeb3Auth(): Promise<void> {
  // Full connector-level cleanup — mirrors the pre-flight cleanup in
  // connectToSocialProvider(). Just calling web3authInstance.logout() is not
  // enough: the WsEmbed recovery path leaves web3authInstance.connected ===
  // false even after a successful social login, so a bare `if (connected)`
  // guard skips clearAuthInstance() here on exactly the logins that need it
  // most. Without it, the Sapphire session and BrowserStorage.instanceMap
  // survive logout, and the next login can silently reuse the old session's
  // private key instead of running a fresh DKG — the same "session bleeding"
  // bug documented above for the login path.
  // Capture the instance we're tearing down so the cleanup below can't
  // clobber a *different* instance that a concurrent initWeb3Auth() call
  // (e.g. the login modal reopening mid-logout) may have assigned to the
  // module-level web3authInstance variable while we were awaiting cleanup.
  const instanceAtStart = web3authInstance;

  if (instanceAtStart) {
    const authConnector = (instanceAtStart as any).connectors?.find((c: any) => c.name === WALLET_CONNECTORS.AUTH);
    if (authConnector) {
      try {
        await authConnector.disconnect?.();
      } catch (e) {
        console.warn('[Web3Auth] Logout: auth connector disconnect failed (non-blocking):', String(e).slice(0, 80));
      }
      if (authConnector.authInstance) {
        await clearAuthInstance(authConnector.authInstance);
      }
    }
    if (instanceAtStart.connected) {
      try {
        await instanceAtStart.logout();
      } catch (e) {
        console.warn('[Web3Auth] Logout: web3authInstance.logout() failed (non-blocking):', String(e).slice(0, 80));
      }
    }
  }

  // Null the module-level singletons so the next initWeb3Auth() creates a
  // fresh instance instead of its early-return guard reusing this stale one
  // (status stays "ready" even though the session was just torn down) — but
  // only if nothing replaced them while we were awaiting cleanup above.
  if (web3authInstance === instanceAtStart) {
    web3authInstance = null;
    isInitializing = false;
    initPromise = null;
  }
