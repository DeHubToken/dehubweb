...
export async function loadPaymentBalances(assets: PaymentAsset[], wallet: string, solana?: string) {
  ...
  return readPaymentBalances(assets, wallet, solana || getCachedSolanaAddress() || undefined, { base: CHAIN_CONFIGS[8453].rpcUrl, eth: CHAIN_CONFIGS[1].rpcUrl, bsc: CHAIN_CONFIGS[56].rpcUrl, robinhood: CHAIN_CONFIGS[4663]?.rpcUrl, sol: solanaRpcUrl() });
}
