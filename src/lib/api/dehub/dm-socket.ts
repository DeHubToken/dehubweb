...
function getDmSocket(): Socket {
  ...
  if (!dmSocket) {
    ...
    // Debug: log all incoming events from server (stringify error payload for
    // visibility). Errors always log; the per-event firehose is dev-only —
    // in production it serialized every DM event to the console.
...
export function onForwardMessage(cb: (msg: DmMessage) => void): () => void {
  return addPersistentListener('forwardMessage', cb as AnyFn);
}
