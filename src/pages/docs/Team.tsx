              ...
              {member.socials.length > 0 && (
                <div className="flex flex-wrap gap-3 pt-6 border-t border-border">
                  {member.socials.map((social, idx) => {
                    ...
                    return (
                      <Button key={idx} variant="outline" size="sm" asChild className="flex-1 min-w-0 bg-background text-foreground border-border hover:bg-background/90">
                        <a href={social.url} className="flex items-center justify-center" target="_blank" rel="noopener noreferrer" aria-label={social.platform}>
                          {social.platform === 'TikTok' ? (
                            ...
                            <img src={xLogo} alt="X" className="w-4 h-4 invert-0 dark:invert" />
