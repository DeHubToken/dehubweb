...
export default function BridgePage() {
  ...
  const { data: allTokens, refetch: refetchTokens } = useAllChainsTokens();
  ...
  const sourceBalance = direction === 'base-to-bnb'
    ? parseFloat(baseDHB?.balance ?? '0')
    : parseFloat(bnbDHB?.balance ?? '0');
  ...
  return (
    <div className={cn("min-h-screen pb-24 px-3 sm:px-4 max-w-5xl mx-auto", isCollapsed && "pt-16 md:pt-0")}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        <StatCard label="Base Balance" value={formatNumber(parseFloat(baseDHB?.balance ?? '0'), 2)} subtitle="DHB on Base" delay={0} />
        <StatCard label="BNB Chain Balance" value={formatNumber(parseFloat(bnbDHB?.balance ?? '0'), 2)} subtitle="DHB on BNB Chain" delay={0.05} />
      </div>
