...
export default function BridgePage() {
  ...
  const sourceToken = direction === 'base-to-bnb' ? baseDHB : bnbDHB;
  const sourceBalanceRaw = sourceToken
    ? formatUnits(sourceToken.balance, sourceToken.decimals)
    : '0';
  ...
  return (
    <div className={cn("min-h-screen pb-24 px-3 sm:px-4 max-w-5xl mx-auto", isCollapsed && "pt-16 md:pt-0")}>
      ...
      <h1 className="text-xs text-zinc-500 mb-2 px-3 sm:px-4 pt-2 font-medium">DeHub Bridge — Decentralised Social Media, Censorship Resistant & Freedom of Speech</h1>
      {/* Header */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
