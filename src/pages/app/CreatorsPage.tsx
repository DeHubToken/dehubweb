...
function CreatorsPageInner() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // The table has no tiktok_username column — fold it into other_socials
      // so the answer isn't silently dropped.
      const tiktok = formData.tiktok_username.trim();
      const otherSocials = [
        tiktok ? `TikTok: ${tiktok}` : null,
        formData.other_socials.trim() || null,
      ].filter(Boolean).join(' | ');

      const { error } = await supabase
        .from('creator_applications')
        .insert({
          x_username: formData.x_username.trim() || null,
          youtube_username: formData.youtube_username.trim() || null,
          twitch_username: formData.twitch_username.trim() || null,
          instagram_username: formData.instagram_username.trim() || null,
          total_follower_reach: formData.total_follower_reach.trim(),
          other_socials: otherSocials || null,
          email: formData.email.trim(),
          expected_compensation: formData.expected_compensation.trim(),
        });
  ...
  // Random positions must be stable across renders — inline Math.random()
  // made all 80 dots jump on every keystroke in the form.
  const dots = useMemo(
    () =>
  ...
  return (
    <div data-glass-page className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        {dots.map((dot, i) => (
          <div key={i} className="absolute" style={dot} />
        ))}
        <div 
