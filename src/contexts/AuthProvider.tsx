...
function mapSocialProvider(provider: SocialProvider): string | null {
  switch (provider) {
    ...
    // Not a Supabase Auth provider and never will be — Telegram has no OAuth 2
    // endpoint. It has its own path: connectWithTelegram.
    case 'telegram': return null;
    default: return null;
  }
}
...
async function getSupabaseAuthMeta(): Promise<Web3AuthMeta | undefined> {
  try {
    ...
    // they can sign in via password (see verify-phone-otp, telegram-auth) —
    // never a real address, so it must never surface as "the user's email".
    // Matched on the suffix, not the one hostname, so the next provider that
    // needs the same trick does not leak on day one.
    const realEmail = u.email?.endsWith('.dehub.internal') ? undefined : u.email;
    // Both of those sign in through the password grant, so app_metadata says
    // "email" for them; the metadata the edge function stamped is the truth.
    const typeOfLogin =
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      if (isWagmiConnected && wagmiAddress && !isLoading) {
        ...
        if (walletAddress && walletAddress.toLowerCase() !== wagmiAddress.toLowerCase()) {
            ...
            if (!wagmiAuthIntentRef.current) {
              ...
              // connection, keep the session, and say nothing: the switch is
              // the wallet's business, not a fault to announce. The log row is
              // what answers "why did my wallet disconnect".
  ...
  useEffect(() => {
    if (telegramResumeRef.current) return;
    const raw = takeStoredTelegramResult();
    if (!raw) return;
    telegramResumeRef.current = true;
    ...
    // Once, on mount: everything it touches is a ref or a setter.
  }, []);
