...
export function useFeedPrefetch(isHomeFeedLoaded: boolean) {
  ...
  useEffect(() => {
    ...
    // Data-Saver / slow network: don't speculatively warm 5 other feeds the user
    // may never open — that background burst is pure waste on a metered connection.
    if (getLiteMode()) return;
