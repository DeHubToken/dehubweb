...
export function useStories({ enrichAvatars = true }: { enrichAvatars?: boolean } = {}) {
  ...
  const { data: enrichedStories = activeStories } = useQuery({
    ...
    // enrichAvatars=false skips the per-username account_info fan-out —
    // callers that only need the ring/viewer (profile page) pass false.
    enabled: enrichAvatars && activeStories.length > 0 && activeStories.some(s => !!s.username),
