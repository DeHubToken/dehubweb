...
export function OptimisticPostsProvider({ children }: { children: ReactNode }) {
  ...
  // Callbacks are stable (useCallback []); memoize the wrapper object too so
  // this app-wide provider only re-renders consumers when the posts change.
  const value = useMemo(
    () => ({ optimisticPosts, addOptimisticPost, clearOptimisticPosts, removeOptimisticPost }),
    [optimisticPosts, addOptimisticPost, clearOptimisticPosts, removeOptimisticPost],
  );

  return (
    <OptimisticPostsContext.Provider value={value}>
      {children}
