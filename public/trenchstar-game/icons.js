...
export function mountIcons(){
  const roots=['tsNav','btnRow','tsFocus','tsDialog','setPanel','alertPanel','walletPanel','csBox','pkBox','srBox'].map(id=>document.getElementById(id)).filter(Boolean);
  function decorate(){
    for(const root of roots)for(const button of root.querySelectorAll('button')){
      if(button.querySelector('.tsIcon'))continue;
      const label=button.textContent.trim().toLowerCase();
      const name=ids[button.id]||tabs[button.dataset.tab]||(button.dataset.delete!==undefined?'trash':button.dataset.load!==undefined?'desk':labels[label]);
      if(!name)continue;
      if(root.id==='btnRow')for(const node of [...button.childNodes])if(node.nodeType===3)node.textContent=node.textContent.replace(/^[^a-zA-Z]+/,'');
      if(button.id==='btnAlerts')button.textContent='Local alerts';
      button.insertAdjacentHTML('afterbegin',icon(name));button.classList.add('tsIconButton');
    }
